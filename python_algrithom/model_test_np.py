...
class CNN_model(object):
	...
	def model(self, X):
		...
		print("time used for first conv: ", time.time() - begin)
		begin = time.time()
		A1_pool = self.pool_forward(A1, 2, 2)
		print("time used for first pool: ", time.time() - begin)
		begin = time.time()
		A2 = self.conv_forward_opencv(A1_pool, self.W_conv2, self.b_conv2, 1, 2)
		print("time used for second conv: ", time.time() - begin)
		begin = time.time()
		A2_pool = self.pool_forward(A2, 2, 2)
		A2_pool = A2_pool.reshape((A2_pool.shape[0], -1))
		print("time used for second pool: ", time.time() - begin)
		begin = time.time()
		A3 = self.dense_forward(A2_pool, self.Wfc1, self.bfc1)
		print("time used for fully connection: ", time.time() - begin)
		begin = time.time()
		A4 = self.dense_forward(A3, self.Wfc2, self.bfc2, 'sigmoid')
		print("time used for fully connection2: ", time.time() - begin)
		return A4
